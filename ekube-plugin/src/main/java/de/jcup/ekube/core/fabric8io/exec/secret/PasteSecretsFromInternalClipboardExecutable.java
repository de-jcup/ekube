...
class PasteSecretsFromInternalClipboardExecutable implements Fabric8ioSafeExecutable<SecretsContainer,Void> {
     ...
     */
    PasteSecretsFromInternalClipboardExecutable(SecretsSupport secretsSupport) {
    ...
    public Void execute(EKubeContext context, KubernetesClient client, SecretsContainer kubeElement, ExecutionParameters parameters) {
        if (this.secretsSupport.encryptedClipboardValue==null) {
            return null;
        }
        ...
        TreeMap<String,KeyValueMap> fromClipboard = this.secretsSupport.copiedSecretsAccess.unseal(this.secretsSupport.encryptedClipboardValue);
        map.putAll(fromClipboard); // will overwrite...
        
        
        
        this.secretsSupport.encryptedClipboardValue=null; // drop this 
        return null;
    }
}